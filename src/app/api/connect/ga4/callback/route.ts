...
export async function GET(request: NextRequest) {
  ...
  if (errorMsg) {
    return NextResponse.redirect(new URL(`/dashboard/connectors?error=Google Analytics authorization failed: ${errorMsg}`, appUrl));
  }

  if (!code) {
    return NextResponse.redirect(new URL("/dashboard/connectors?error=Google Analytics authorization code not found", appUrl));
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(new URL("/dashboard/connectors?error=Google Analytics credentials not configured on server", appUrl));
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(
        new URL(`/dashboard/connectors?error=Failed to exchange Google token: ${errorData.error_description || tokenResponse.statusText}`, appUrl)
      );
    }
    ...
    return NextResponse.redirect(new URL("/dashboard/connectors?message=Google Analytics 4 connected and synced successfully!", appUrl));
  } catch (error: any) {
